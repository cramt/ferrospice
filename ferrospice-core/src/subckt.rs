...
use ferrospice_netlist::{Element, ElementKind, Expr, Item, Netlist, Param, SubcktDef};
use thiserror::Error;
...
fn resolve_source(
    source: &ferrospice_netlist::Source,
    param_map: &BTreeMap<String, f64>,
) -> ferrospice_netlist::Source {
    ferrospice_netlist::Source {
        dc: source.dc.as_ref().map(|e| resolve_expr(e, param_map)),
        ac: source.ac.as_ref().map(|ac| ferrospice_netlist::AcSpec {
